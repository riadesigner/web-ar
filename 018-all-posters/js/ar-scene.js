...
var AR_SCENE = {
	...
	add_from_queue:function() {
		...
		// do this at once
		var foo = sets.foo; 
		var patternName = sets.patternName;
		var barcodeValue = sets.num;
		...
		markerRoot.name='marker'+this.QUEUE_SETS.length;
		this.ALL_MARKERS[markerRoot.name]={markerRoot:markerRoot,visible:false};
		this.scene.add( markerRoot );
		

		if(patternName=='barcode'){
			...
			var arMarkerControls = new THREEx.ArMarkerControls(this.arToolkitContext, markerRoot, {
				type:'pattern',
				smooth: true,
				patternUrl: THREEx.ArToolkitContext.baseURL + patternName,
		...
		this.onRenderFcts.push( ()=> {
			for(var i in this.ALL_MARKERS){
				...
				if(m.markerRoot.visible!==m.visible){					
					...
					if(m.visible==true){
						document.dispatchEvent(new CustomEvent('markerFound',{detail:{marker:m.markerRoot}}))
					}else{
						document.dispatchEvent(new CustomEvent('markerLost',{detail:{marker:m.markerRoot}}))
					}
				}				
			}
		});	
